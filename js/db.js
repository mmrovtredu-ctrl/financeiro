// ============================================================
// CAMADA DE DADOS — Supabase com fallback local (localStorage)
// Expõe window.Store
// ============================================================
(function () {
  ...
  function freshLocal() {
    return {
      settings: { meta: 1000, saldo_inicial: 0.13 },
      txs: seedTxs().map((t) => ({ id: uid(), ...t })),
      savings: [],
    };
  }
  ...
  const Store = {
    ...
    async init() {
      if (this.mode === "supabase") {
        try {
          ...
          console.warn("Falha no Supabase, usando modo local:", e);
          this.mode = "local";
        }
      }
      this.state = loadLocal() || freshLocal();
      saveLocal(this.state);
    },

    async _sbLoad() {
      // settings
      let { data: st } = await sb.from("settings").select("*").eq("id", 1).maybeSingle();
      if (!st) {
        await sb.from("settings").insert({ id: 1, meta: 1000, saldo_inicial: 0.13 });
        st = { meta: 1000, saldo_inicial: 0.13 };
      }
      this.state.settings = { meta: Number(st.meta), saldo_inicial: Number(st.saldo_inicial) };

      // transactions (seed se vazio)
    ...
    async addTx(tx) {
      if (this.mode === "supabase") {
        ...
        this.state.txs.push(this._num(data));
      } else {
        this.state.txs.push({ id: uid(), ...tx });
        saveLocal(this.state);
      }
    },
    async delTx(id) {
      if (this.mode === "supabase") await sb.from("transactions").delete().eq("id", id);
      this.state.txs = this.state.txs.filter((t) => t.id !== id);
      if (this.mode === "local") saveLocal(this.state);
    },
    async addSaving(s) {
      if (this.mode === "supabase") {
        const { data, error } = await sb.from("savings").insert(s).select().single();
        if (error) throw error;
        this.state.savings.push(this._num(data));
      } else {
        this.state.savings.push({ id: uid(), ...s });
        saveLocal(this.state);
      }
    },
    async setMeta(meta) {
      if (this.mode === "supabase") await sb.from("settings").update({ meta, updated_at: new Date().toISOString() }).eq("id", 1);
    ...
    async reset() {
      if (this.mode === "supabase") {
        ...
        await sb.from("savings").delete().neq("id", "00000000-0000-0000-0000-000000000000");
        await sb.from("settings").update({ meta: 1000, saldo_inicial: 0.13 }).eq("id", 1);
        await sb.from("transactions").insert(seedTxs());
        await this._sbLoad();
      } else {
        this.state = freshLocal();
        saveLocal(this.state);
      }
    },
  };
