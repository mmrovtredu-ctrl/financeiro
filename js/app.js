// ============================================================
// APP — interface, render e gráficos
// ============================================================
...
function render() {
  ...
  const pt = poupTotal();
  $("dPoup").innerHTML = pt > 0
    ? '<span style="color:var(--green)">' + fmt(pt) + "</span>"
    : '<span style="color:var(--red)">R$ 0,00 — comece hoje</span>';
  ...
  $("txBody").innerHTML = rows.map((t) => `
    <tr>
      ...
      <td><button class="del" onclick="delTx('${t.id}')" aria-label="Apagar">✕</button></td>
    </tr>`).join("") || `<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:24px">Nenhum lançamento.</td></tr>`;
  ...
  $("depList").innerHTML = Store.state.savings.slice().reverse().map((d) =>
    `<div class="d"><span>${br(d.date)} · ${esc(d.description || "Depósito")}</span><b style="color:var(--green)">${fmt(d.amount)}</b></div>`
  ).join("") || `<div style="color:var(--muted);font-size:14px;padding:8px 0">Ainda não há depósitos. Guarde o primeiro valor acima — todo começo conta.</div>`;
...
function drawCharts() {
  ...
  charts.cat = new Chart($("catChart"), {
    type: "doughnut",
    data: { labels: ce.map((e) => e[0]), datasets: [{ data: ce.map((e) => e[1]), backgroundColor: palette, borderWidth: 2, borderColor: "#fff" }] },
    options: { responsive: true, maintainAspectRatio: false, cutout: "58%", plugins: { legend: { position: "bottom", labels: { boxWidth: 11, boxHeight: 11, padding: 9, usePointStyle: true } }, tooltip: { callbacks: { label: (c) => " " + c.label + ": " + fmt(c.parsed) } } } },
  });
  charts.io = new Chart($("ioChart"), {
    type: "bar",
    data: { labels: ["Entradas", "Saídas"], datasets: [{ data: [ent, sai], backgroundColor: ["#047857", "#DC2626"], borderRadius: 8, maxBarThickness: 70 }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c) => " " + fmt(c.parsed.y) } } }, scales: { y: { ticks: { callback: (v) => "R$ " + v / 1000 + "k" }, grid: { color: "#EEF1F4" } }, x: { grid: { display: false } } } },
  });
  const se = Object.entries(src).sort((a, b) => b[1] - a[1]);
  charts.src = new Chart($("srcChart"), {
    type: "doughnut",
    data: { labels: se.map((e) => e[0]), datasets: [{ data: se.map((e) => e[1]), backgroundColor: palette, borderWidth: 2, borderColor: "#fff" }] },
    options: { responsive: true, maintainAspectRatio: false, cutout: "58%", plugins: { legend: { position: "bottom", labels: { boxWidth: 11, boxHeight: 11, padding: 9, usePointStyle: true } }, tooltip: { callbacks: { label: (c) => " " + c.label + ": " + fmt(c.parsed) } } } },
  });
}
...
function addTx() {
  ...
  guard(async () => {
    await Store.addTx({ date, description, category, side, type, amount });
    $("fDesc").value = ""; $("fAmount").value = "";
  });
}
function delTx(id) { guard(() => Store.delTx(id)); }
function addDep() {
  const a = parseFloat($("depAmount").value);
  if (!a || a <= 0) { alert("Informe um valor para guardar."); return; }
  guard(async () => { await Store.addSaving({ date: new Date().toISOString().slice(0, 10), description: "Depósito", amount: a }); $("depAmount").value = ""; });
}
function setMeta() {
  const m = parseFloat($("metaAmount").value);
  if (!m || m <= 0) { alert("Informe uma meta válida."); return; }
  guard(async () => { await Store.setMeta(m); $("metaAmount").value = ""; });
}
...
function exportData() {
  ...
  const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "controle-financeiro.json"; a.click();
}
...
(async function () {
  ...
  try {
    ...
    if (Store.mode === "supabase") { badge.textContent = "Supabase conectado"; badge.classList.add("ok"); }
    else { badge.textContent = "Modo local (neste aparelho)"; }
